...
object Executer:
  def executeExperiment(inputSizes: List[Int], algorithm: CheckSum): List[(Int, Long)] =
    val path = ExperimentController.PATH_FOR_TEST_FILES + DataManager.TEST_FILE_PREFIX

    @tailrec
    def executeExperimentAux(inputSizes: List[Int], accResults: List[(Int, Long)] = Nil): List[(Int, Long)] =
      inputSizes match
        case Nil => accResults
        case inputSize :: t =>
  ...
  def exportResultsToCSV(testedAlgorithmName: String, results: List[(Int, Long)]): Unit = {
